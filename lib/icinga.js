...
var ProcessResponse = function (response) {
  ...
  if (response == null || typeof response !== "object") {
    var errmsg = "Response is null. Please check your configuration. Visit https://github.com/ErikWegner/imoin for further assistance.";
    emit(target, EventNames.GenericError, errmsg);
    return;
  }
  ...
  if (response.cgi_json_version) {
    ...
    if (version_parts.length >= 2) {
      ...
      if (major_version === 1) {
        if (minor_version >= 8) {
          ProcessResponse_1_10(response, status);
          processed = true;
        } else if (minor_version === 6) {
          ProcessResponse_1_6(response, status);
          processed = true;
        }
      }
    }
