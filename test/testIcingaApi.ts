...
import { IcingaApi } from '../scripts/monitors/index.js';
...
describe('IcingaApi', () => {
  const baseurl =
    'testurl/v1/objects/###?' + 'attrs=display_name&attrs=last_check_result';
  ...
  Object.keys(filterSettingsTests).forEach((description) => {
    ...
    describe(description, () => {
      ...
      it(
        'should not set ' +
          description +
          ' on host when attribute is not in response',
        () => {
          const e = new MockAbstractEnvironment();
          const settings = buildSettings();
