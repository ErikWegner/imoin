...
export class IcingaApi extends AbstractMonitor {
  public static processData(
    ...
    servicedata.results.forEach((jsonservice) => {
      const host =
        hostByName[jsonservice.name.substr(0, jsonservice.name.indexOf('!'))];
      if (host) {
        const service = new Service(jsonservice.attrs.display_name);
  ...
  public fetchStatus(): Promise<MonitorData> {
    return new Promise<MonitorData>((resolve) => {
      ...
      Promise.all([hostsrequest, servicesrequest])
        .then((a: string[]) => {
          const hostdata = JSON.parse(a[0]) as IIcinga2HostJsonData;
          const servicedata = JSON.parse(a[1]) as IIcinga2ServiceJsonData;
